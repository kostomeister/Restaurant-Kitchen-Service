...
class DishTypeDetailViewTest(TestCase):
    ...
    def test_dish_type_detail_view(self):
        response = self.client.get(
            reverse("kitchen:dish-type-detail", args=[self.dish_type.id])
        )
        self.assertEqual(response.status_code, 200)
...
class DishTypeCreateViewTest(TestCase):
    ...
    def test_dish_type_create_view(self):
        ...
        response = self.client.post(
            reverse("kitchen:dish-type-create"), data={"name": "Test"}
        )
    ...
    def test_dish_type_create_view_requires_staff(self):
        self.client.force_login(
            get_user_model().objects.create_user(
                username="non_staff_user", password="test_password", is_staff=False
            )
        )
...
class DishTypeListViewSearchTest(TestCase):
    ...
    def test_dish_type_list_view_with_search(self):
        response = self.client.get(
            reverse("kitchen:dish-types-list"), {"dish_type": "Test"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_dish_type_list_view_with_invalid_search(self):
        response = self.client.get(
            reverse("kitchen:dish-types-list"), {"dish_type": "NonExistent"}
        )
        self.assertEqual(response.status_code, 200)
...
class DishCreateViewTest(TestCase):
    ...
    def test_dish_create_view(self):
        ...
        response = self.client.post(
            self.url,
            data={"name": "Ramen", "description": "Delicious ramen", "price": "10.99"},
        )
        self.assertEqual(response.status_code, 302)
...
class DishUpdateViewTest(TestCase):
    def setUp(self):
        ...
        self.dish = Dish.objects.create(
            name="Tempura Udon", description="Tasty udon dish", price="15.99"
        )
...
class DishDeleteViewTest(TestCase):
    def setUp(self):
        ...
        self.dish = Dish.objects.create(
            name="Teriyaki Chicken", description="Delicious chicken dish", price="12.99"
        )
